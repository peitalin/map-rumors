...
export class PokemonStats extends React.Component<PokemonStatsProps, any> {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.data.loading && !this.props.data.Pokemon) {
  ...
  render() {
    if (this.props.data.error) {
      ...
      return <Title><div>PokemonStats: GraphQL Errored.</div></Title>
    }
    ...
    let { attack, defense, speed, hp, spAtk, spDef } = this.props.data.Pokemon
    let baseStats = [
          ...
          <div>
            ...
            <Card title={ name }>
              ...
              <div style={{ right: '2%', top: '2%', position: 'absolute' }}>
                ...
                { this.state.redirectToMap && (<Redirect to={'/map/pokemon'}/>) }
              </div>
              ...
              <div className='flex '>
                ...
                <div style={{flexBasis: '50%'}}>
                  ...
                  <AddPokemon data={ this.props.data } />
                </div>
              </div>
            </Card>
...
export const PokemonStatsQuery = gql`
query PokemonStatsQuery($pname: String!) {
  Pokemon(name: $pname) {
    id
    name
    img
...
const PokemonStatsQueryOptions = {
  options: (ownProps) => {
    return ({
      variables: {
        pname: ownProps.match.params.pname
      }
    })
  }
}

export default graphql(PokemonStatsQuery, PokemonStatsQueryOptions)( PokemonStats )
